...
func BigRandomizer(b1:Big,b2:Big):
	var sBig:Big
	var bBig:Big
	var rBig:Big
	var dBig:Big
	...
	if(sBig.exponent == bBig.exponent):
		rBig.exponent = sBig.exponent
		rBig.mantissa = randf_range(sBig.mantissa,bBig.mantissa)
		return rBig
	dBig = dBig.subtract(bBig.add(bBig,Big.new(1,0)),sBig)
	#work on exponent odds for each set.
	#works for variable differences by 1 order of magnitude
	if(bBig.exponent-sBig.exponent ==1):
		var tBig:Big
		var tBig2:Big
		tBig2 = Big.new(9.99999999999,bBig.exponent-1)
		tBig = Big.new(1,0)
		tBig = tBig.subtract(bBig,tBig2)
		tBig = tBig.add(tBig,Big.new(1,0))
		print(tBig.toScientific())
		tBig2 = dBig.divide(tBig,dBig)
		print(dBig.toScientific())
		print(tBig2.toScientific())
		if(tBig2.isGreaterThan(randf())):
			rBig.exponent = bBig.exponent
			rBig.mantissa = randf_range(1,bBig.mantissa)
		else:
			rBig.exponent = sBig.exponent
			rBig.mantissa = randf_range(sBig.mantissa,9.999999999)
		return rBig
	#works for variable difference by 2 orders of magnitude
	if(bBig.exponent-sBig.exponent ==2):
		
		var tBig:Big
		var tBig2:Big
		var tBig3:Big
		tBig2 = Big.new(9.99999999999,bBig.exponent-1)
		tBig3 = Big.new(9.99999999999,bBig.exponent-2)
		tBig = Big.new(1,0)
		tBig = tBig.subtract(bBig,tBig2)
		tBig = tBig.add(tBig,Big.new(1,0))
		print(tBig.toScientific())
		...
		print(tBig2.toScientific())
		#second threshold
		tBig = tBig.subtract(Big.new(9.99999999999,bBig.exponent-1),tBig3)
		print(tBig.toScientific())
		tBig3 = dBig.divide(tBig,dBig)
		#may need to reconsider what is what, tBig2 is correct, but tBid 3 may not
		#TBig3 will always be the most common position, or the second rarest
		#Tbig2 can be in any position, being most or least common or almost equally common as tBig3 as it refers to largest position
		
		var f = randf()
		print(f)
		
		if(tBig3.isGreaterThanOrEqualTo(tBig2)):
			
			print(tBig3.add(tBig3,tBig2).toScientific())
			if(tBig2.isGreaterThanOrEqualTo(f)):
				
				rBig.exponent = bBig.exponent
				rBig.mantissa = randf_range(1,bBig.mantissa)
				return rBig
			
			if(tBig3.add(tBig3,tBig2).isGreaterThanOrEqualTo(f)):
				
				rBig.exponent = bBig.exponent-1
				rBig.mantissa = randf_range(1,9.9999999999999999999)
				return rBig
			
			rBig.exponent = bBig.exponent -2
			rBig.mantissa = randf_range(sBig.mantissa,9.9999999999999)
			return rBig
		if(tBig2.isGreaterThan(tBig3)):
			
			print(tBig3.add(tBig3,tBig2).toScientific())
			if(tBig3.isGreaterThanOrEqualTo(f)):
				rBig.exponent = bBig.exponent -1
				rBig.mantissa = randf_range(1,9.99999999999999999)
				return rBig
			
			if(tBig2.add(tBig3,tBig2).toScientific()):
				rBig.exponent = bBig.exponent
				rBig.mantissa = randf_range(1, bBig.mantissa)
				return rBig
			
			rBig.exponent = bBig.exponent -2
			rBig.mantissa = randf_range(sBig.mantissa, 9.99999999999999)
			
			return rBig
	
	
	if(bBig.exponent - sBig.exponent ==3):
		...
		tBig2 = Big.new(9.99999999999,bBig.exponent-1)
		tBig3 = Big.new(9.99999999999,bBig.exponent-2)
		tBig4 = Big.new(9.99999999999,bBig.exponent-3)
		tBig = Big.new(1,0)
		tBig = tBig.subtract(bBig,tBig2)
		tBig = tBig.add(tBig,Big.new(1,0))
		print(tBig.toScientific())
		...
		tBig3 = dBig.divide(tBig,dBig)
		#third threshold
		print(tBig3.toScientific())
		tBig = tBig.subtract(Big.new(9.999999999999,bBig.exponent-2),tBig4)
		tBig4 = dBig.divide(tBig,dBig)
		print(tBig4.toScientific())
		#may need to reconsider what is what, tBig2 is correct, but tBid 3 may not
		#TBig3 will always be the most common position, or the second rarest
		#Tbig2 can be in any position, being most or least common or almost equally common as tBig3 as it refers to largest position
		#tBig2 represents the highest e, tBig3 represents the ssecond lowest e, and then tbig4 represents the third lowest e
		...
		if(tBig2.isGreaterThanOrEqualTo(f)):
			...
			rBig.mantissa = randf_range(1,bBig.mantissa)
			return rBig
		if(tBig3.add(tBig2,tBig3).isGreaterThanOrEqualTo(f)):
			rBig.exponent = bBig.exponent -1
			rBig.mantissa = randf_range(1,9.999999999999999)
			return rBig
		print(tBig3.toScientific())
		
		if(tBig4.add(tBig2,tBig3.add(tBig3,tBig4)).isGreaterThanOrEqualTo(f)):
			rBig.exponent = bBig.exponent -2
			rBig.mantissa = randf_range(1,9.999999999999999)
			return rBig
		rBig.exponent = sBig.exponent
	...
	if(bBig.exponent - sBig.exponent == 4):
		var tBig:Big
		var tBig2:Big
		var tBig3:Big
		var tBig4:Big
		...
		tBig3 = Big.new(9.99999999999,bBig.exponent-2)
		tBig4 = Big.new(9.99999999999,bBig.exponent-3)
		tBig5 = Big.new(9.99999999999,bBig.exponent-4)
		tBig = Big.new(1,0)
		tBig = tBig.subtract(bBig,tBig2)
		tBig = tBig.add(tBig,Big.new(1,0))
		print(tBig.toScientific())
		...
		tBig4 = dBig.divide(tBig,dBig)
		print(tBig4.toScientific())
		#fourth and final considered threshold
		tBig = tBig.subtract(Big.new(9.999999999999,bBig.exponent-3),tBig5)
		tBig5 = dBig.divide(tBig,dBig)
		print(tBig5.toScientific())
		#may need to reconsider what is what, tBig2 is correct, but tBid 3 may not
		#TBig3 will always be the most common position, or the second rarest
		#Tbig2 can be in any position, being most or least common or almost equally common as tBig3 as it refers to largest position
		#tBig2 represents the highest e, tBig3 represents the ssecond lowest e, and then tbig4 represents the third lowest e
		
		print("Sum of values are "+(tBig4.add(tBig2,tBig3.add(tBig3,tBig4))).toScientific())
		var f = randf()
		print(f)
		if(tBig2.isGreaterThanOrEqualTo(f)):
			rBig.exponent = bBig.exponent
		...
		if(tBig4.add(tBig2,tBig3.add(tBig3,tBig4)).isGreaterThanOrEqualTo(f)):
			...
			rBig.mantissa = randf_range(1,9.999999999999999)
			return rBig
		if(tBig5.add(tBig2.add(tBig2,tBig3),tBig4.add(tBig4,tBig5)).isGreaterThanOrEqualTo(f)):
			rBig.exponent = bBig.exponent -3
			rBig.mantissa = randf_range(1,9.999999999999999)
			return rBig
		rBig.exponent = sBig.exponent
		rBig.mantissa = randf_range(sBig.mantissa, 9.99999999999999)
	...
	if(bBig.exponent-sBig.exponent >4):
		...
		if(tBig5.add(tBig2.add(tBig2,tBig3),tBig4.add(tBig4,tBig5)).isGreaterThanOrEqualTo(f)):
			...
			rBig.mantissa = randf_range(1,9.999999999999999)
			return rBig
		rBig.exponent = bBig.exponent-4
		rBig.mantissa = randf_range(1, 9.99999999999999)
		return rBig
